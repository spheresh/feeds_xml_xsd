...
//        return new FeedsParserResult($rows, $source->feed_nid);
    ...
    {
        if(empty($values['xsd'])){
            form_set_error('xsd', t("Please enter a value for the XSD file"));
            return;
        }
        $xsd = @file_get_contents($values['xsd']);
        if($xsd === false){
            form_set_error('xsd', t("Input file doesn't exist"));
            return;
        }
        $parser = new XsdToObject();
        $result = $parser->parse($xsd);
        if(count($result)==0){
            form_set_error('xsd', t("This doesn't seen to be a valid XSD schema"));
        }
